...
def main():
    ...
    print('Test case 1')
    t1 = datetime.now()
    state = env.reset([1710, 1710, 35168, 1710, 1710])
    print(state.shape)
    act_mask = env.batch_action_mask(0.5)
    state, reward, done = env.batch_step([4, 4, 0, 4, 4])
    print(state.shape, act_mask.shape, reward, done)
    act_mask = env.batch_action_mask(0.5)
    state, reward, done = env.batch_step([61, 61, 0, 61, 61])
    print(state.shape, act_mask.shape, reward, done)
    act_mask = env.batch_action_mask(0.5)
    state, reward, done = env.batch_step([0, 0, 0, 0, 0])
    print(state.shape, act_mask.shape, reward, done)
    t2 = datetime.now()
    env.print_path()
    ...
    for i in range(10):
        ...
        act_idx = random.choice(acts)
        state, reward, _ = env.step(act_idx)
        rewards += reward
    print(rewards, env._path)
    ...
    for uid in range(1000):
        ...
        while not done:
            acts = env.get_actions()
            act_idx = random.choice(acts)
            state, reward, done = env.step(act_idx)
            rewards.append(reward)
        if np.sum(rewards) > 0:
            print('==', uid, rewards, env._path, '==')

    return

    # Test case 4
    print('\nTest case 4')
    env = KGEnvironment(dataset_str, max_acts, max_path_len, mode='test')
    env.reset(0)
    print(env._target_pids)
